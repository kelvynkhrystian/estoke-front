...
export default function Estoque() {
  ...
  const totais = useMemo(() => {
    ...
    const totalEstoque = itensAtivos.reduce(
      (acc, item) => acc + Number(item.estoque || 0),
      0,
    );
    const abaixoMinimo = itensAtivos.filter(
      (item) => Number(item.estoque) < Number(item.minimo),
    ).length;
  ...
  async function handleSalvarTransferencia(e) {
    ...
    if (
      Number(transferencia.loja_origem_id) ===
      Number(transferencia.loja_destino_id)
    ) {
    ...
    <>
      <div className="products-page">
        ...
        <div className="products-tabs">
          <button
            className={
              abaAtiva === 'produtos' ? 'active products-tab' : 'products-tab'
            }
          ...
          <button
            className={
              abaAtiva === 'insumos' ? 'active products-tab' : 'products-tab'
            }
        ...
        <div className="stocks-actions">
          <button
            ...
            onClick={() =>
              abrirMovimentacao(abaAtiva === 'produtos' ? 'produto' : 'insumo')
            }
          >
          ...
          <button
            className="products-subtab"
            onClick={() =>
              abrirTransferencia(abaAtiva === 'produtos' ? 'produto' : 'insumo')
            }
          >
          ...
          <div className="modal-box glass" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="modal-form" onSubmit={handleSalvarMovimentacao}>
              ...
              {movimentacao.tipo_movimento === 'saida' &&
                movimentacao.motivo === 'perda' && (
                  <div className="alert-box">
                    <AlertTriangle size={18} />
                    <span>
                      Essa saída será registrada como perda no estoque.
                    </span>
                  </div>
                )}
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setTransferencia({
                        ...transferencia,
                        loja_destino_id: e.target.value,
                      })
                    }
                  >
                    <option value="">Selecione</option>
